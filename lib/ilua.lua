...
local function color(str, color)
  return '\27['..color..'m'..str..'\27[m'
end

local pretty_print_limit = 20
local max_depth = 7
local table_clever = true
local prompt  = color(_VERSION, "1;30")..color(" >> ", 33)
local verbose = false
local strict  = true
...
local function join(tbl,delim,limit,depth)
  limit = limit or pretty_print_limit
  ...
  if is_list then
    ...
    for key,v in pairs(tbl) do
      ...
      res = res..delim..color(key, 32)..' = '..val2str(v)
      k = k + 1
      if k > limit then
        res = res.." ... "
        break
...
function val2str(val)
  ...
  if tp == 'function' then
    return color(tostring(val), 34)
  elseif tp == 'table' then
    if val ~= _G and val.__tostring  then
       return tostring(val)
    else
      return '{'..join(val,', ')..'}'
    end
  ...
  elseif tp == 'number' then
    ...
    if num_prec and (num_all or floor(val) ~= val) then
      ...
      return color(tostring(val), 35)
    end
  elseif tp == "boolean" then
    return color(tostring(val), val and 32 or 31)
  elseif tp == "nil" then
    return color('nil', 36)
  else
    return tostring(val)
...
function _pretty_print(...)
  if arg[1] == nil then oprint(val2str(nil)) end
...
f, e = loadfile(os.getenv("HOME").."/.iluarc")
if (e == nil) then
  f()
end
